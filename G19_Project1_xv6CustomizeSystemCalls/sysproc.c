...
int sys_cps(void)
{
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {
        if (p->state == UNUSED)
            continue;
        cprintf("%d\t%s\t%d\t%s\n", p->pid, states[p->state] ? states[p->state] : "?", p->priority, p->name);
    }
    release(&ptable.lock);
    return 0;
}
...
int sys_waitx(void)
{
    ...
    if (argint(0, &wtime_addr) < 0)
        return -1;
    if (argint(1, &rtime_addr) < 0)
        return -1;
...
{
    int key;
    if (argint(0, &key) < 0 || key < 0 || key >= SHM_NKEYS)
        return -1;
...
static void mutex_table_init(void)
{
...
static struct msg_queue *queue_list_head = (void *)0;
static struct spinlock queue_list_lock;
static int queue_list_initialized = 0;
static int next_queue_id = 1;

static void queue_table_initialize(void)
{
    if (!queue_list_initialized)
    {
        initlock(&queue_list_lock, "msgqueue");
        queue_list_initialized = 1;
    }
}
...
int sys_msgget(void)
{
    ...
    queue_table_initialize();
    acquire(&queue_list_lock);

    struct msg_queue *queue = queue_list_head;
    while (queue)
    {
        // If there is a queue with that key
        if (queue->key == key)
        {
    ...
    // If there is no queue with that key, we have to create a new key
    ...
    initlock(&queue->lock, "msg_queue_lock");

    queue->next = queue_list_head;
    queue_list_head = queue;

    int return_id = queue->id;
    release(&queue_list_lock);
    return return_id;
}
...
int sys_msgsnd(void)
{
    ...
    if (argint(0, &msqid) < 0 || argint(2, &msgsz) < 0 || argint(3, &msgflg) < 0)
        return -1;
    if (argptr(1, &msgp, msgsz + sizeof(long)) < 0)
        return -1;
    ...
    long *user_message_type = (long *)msgp;
    node->msgtyp = *user_message_type;
    node->length = msgsz;
    node->next = (void *)0;
...
int sys_msgrcv(void)
{
    ...
    if (argint(0, &msqid) < 0 || argint(2, &msgsz) < 0 || argint(3, &msgtyp) < 0 || argint(4, &msgflg) < 0)
        return -1;
    ...
    while (1)
    {
        struct msg_node *previousnode = (void *)0;
        struct msg_node *currentnode = queue->head;

        while (currentnode)
        {
            if (msgtyp == 0 || currentnode->msgtyp == msgtyp)
            {
                if (previousnode)
                ...
                int copysz;
                if (currentnode->length < (unsigned int)msgsz)
                    copysz = currentnode->length;
                else
                    copysz = msgsz;

                if (argptr(1, &msgp, sizeof(long) + copysz) < 0)
                {
                    kfree((char *)currentnode);
                    return -1;
                }

                long *user_mtype = (long *)msgp;
                *user_mtype = currentnode->msgtyp;
                if (copysz > 0)
                    memmove(msgp + sizeof(long), currentnode->data, copysz);
...
int sys_msgctl(void)
{
    ...
    if (argint(0, &msqid) < 0 || argint(1, &cmd) < 0)
        return -1;
    ...
    struct msg_queue *previousnode = (void *)0;
    struct msg_queue *currentnode = queue_list_head;

    while (currentnode)
    {
        if (currentnode->id == msqid)
        {
            if (previousnode)
                previousnode->next = currentnode->next;
            else
                queue_list_head = currentnode->next;

            release(&queue_list_lock);
            acquire(&currentnode->lock);

            struct msg_node *node = currentnode->head;
            while (node)
            {
            ...
            currentnode->head = currentnode->tail = (void *)0;

            wakeup(currentnode);
            release(&currentnode->lock);
